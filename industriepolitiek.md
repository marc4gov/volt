...
```mermaid
flowchart TD
    A[Grondstoffen] --> B[Productie]
    B --> C[Producten & Diensten]
    C --> D[Handel]
...
```mermaid
flowchart LR
    subgraph Huidige Supply Chain
        A1[Grondstoffen\n⬆️ CO₂, ⬆️ Energie, ⬆️ Grondstofgebruik] --> B1[Productie\n⬆️ CO₂, ⬆️ Energie]
        B1 --> C1[Producten & Diensten\n⬆️ Afval, ⬇️ Circulariteit]

        A1 --> A1a[Fossiele brandstoffen]
        A1 --> A1b[Metalen]
        A1 --> A1c[Minerale grondstoffen]

        B1 --> B1a[Energie-intensieve processen]
        B1 --> B1b[Bulkchemie]
        B1 --> B1c[Raffinage]

        C1 --> C1a[Wegwerpeconomie]
        C1 --> C1b[Beperkte recycling]
    end

    subgraph Toekomstige Duurzame Supply Chain
        A2[Grondstoffen\n⬇️ CO₂, ⬇️ Energie, ⬇️ Grondstofgebruik] --> B2[Productie\n⬇️ CO₂, ⬇️ Energie]
        B2 --> C2[Producten & Diensten\n⬇️ Afval, ⬆️ Circulariteit]

        A2 --> A2a[Hernieuwbare grondstoffen]
        A2 --> A2b[Kritieke grondstoffen]
        A2 --> A2c[Secundaire grondstoffen]

        B2 --> B2a[Duurzame energie]
        B2 --> B2b[Circulaire processen]
        B2 --> B2c[Energie-efficiëntie]

        C2 --> C2a[Repareerbare producten]
        C2 --> C2b[Gesloten kringen]
        C2 --> C2c[Dienstverlening]
    end

    Huidige -->|Transitie| Toekomstige
```
...
flowchart TD
    ...
    C --> D[Maakindustrie]
    D --> D1[Discrete productie]
    D --> D2[Continue productie]

    D1 --> D1a[Computers, elektronica]
    D1 --> D1b[Machines en apparaten]
    D1 --> D1c[Auto’s, transportmiddelen]
    D1 --> D1d[Meubels, textiel]

    D2 --> D2a[Voedingsmiddelen]
    D2 --> D2b[Fijne chemicaliën]
    D2 --> D2c[Farmaceutica]
    D2 --> D2d[Reparatie & installatie]
```
...
flowchart TD
    ...
    subgraph Transitiepad
        A --> E[Stap 1: Ontwikkel Verduurzamende Technologie]
        E --> E1[Pilotfabrieken in NL]
        E --> E2[Waterstof-DRI]
        E --> E3[CCUS]

        E --> F[Stap 2: Verplaats Productie]
        F --> F1[Naar EU-landen met groene energie]
        F --> F2[Behoud kennis en R&D in NL]

        F --> G[Stap 3: Bouw Vervuilende Productie Af]
        G --> G1[Sluiting niet-verduurzaamde fabrieken]
        G --> G2[Herbestemming terreinen]
    end
```
